trigger UpdateContract on Candidate_Requirement__c(after update) {
	Object_Trigger_Switch__mdt metadataSwitch = Object_Trigger_Switch__mdt.getInstance('ProgramRequirement');
    Boolean runTrigger = False;
    if(metadataSwitch != null && !metadataSwitch.isActive__c && !Test.isRunningTest()){
        runTrigger = True;
    }
    
    if(Test.isRunningTest() && TestClassUtil.isForOldTrigger() == 'OLD' ){
        runTrigger = True;
    }
        ...
        if (setContractIds.size() > 0) {
            ...
            for (Contract objCon : [select id, AccountId, recordTypeId, recordType.Name, Status, Membership_Type__c, ContractTerm, EndDate, StartDate, All_Requirements_Completed__c, All_Requirements_Completed_Date__c from Contract where id in : setContractIds]) {
                ...
                if(objCR.Status__c == 'Completed') {
                    ...
                    else if(objCr.RecordTypeId == examRecordTypeID && objCr.Status__c == 'Completed' && objCon.RecordTypeId ==icbrrRecordTypeID) {
