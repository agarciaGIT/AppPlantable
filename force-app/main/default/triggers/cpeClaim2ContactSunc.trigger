trigger cpeClaim2ContactSunc on CPE_Claim__c (after insert, after update) {
    ...
    List<CPE_Claim__c> lclaim = [Select Id, Name, Credits_Earned__c, ApprovedCredits__c, Pending_Credits__c, Date_of_Completion__c, Candidate_Requirement__c, CreatedDate, Candidate_Requirement__r.Candidate_Commitment__c from CPE_Claim__c where Candidate_Requirement__r.Candidate_Commitment__c in :lcontract];

    List<Contact> lstContacts = new List<Contact>();
    for(Contact objContact : [select Id, Name, AccountId, KPI_Membership_Expiration_Date__c from contact where AccountId in: mapAcctIds.keySet()])
    {
        ...
        Decimal credits=0;
        Decimal creditsSinceLapsed=0;
        Decimal submissionsSinceLapsed=0;
        Decimal firstcreditsSinceLapsed=0;
        ...
        for(Contract con : lcontract) {
            ...
            if(con.AccountId == objContact.AccountId) {
                ...
                system.debug('* * * calc credits for cr: ' + credits + ':' + cr + ':' + creditsSinceLapsed + ':' + submissionsSinceLapsed);
                if(credits != 0)
        ...
        if(currentContract != null) {
            ...
            for(CPE_Claim__c claim : lclaim) {
                ...
                if(claim.Candidate_Requirement__r.Candidate_Commitment__c == currentContract.Id && claim.CreatedDate >= objContact.KPI_Membership_Expiration_Date__c) {
                    creditsSinceLapsed+=claim.Credits_Earned__c;
                    submissionsSinceLapsed++;
                }
                
            }            
            objContact.KPI_CPE_Credits__c = credits;  
            if(trigger.isinsert) {
                objContact.CPD_Total_Credits_Submitted_Since_Lapsed__c = creditsSinceLapsed;  
                objContact.CPD_Num_of_Submission_Since_Lapsed__c = submissionsSinceLapsed;  
