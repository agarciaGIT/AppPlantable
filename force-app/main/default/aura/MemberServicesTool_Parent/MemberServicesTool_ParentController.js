({
    onOptionSelect: function (cmp, evt, helper) {
        ...
        if(selectedValue == 'Defer/Undefer' || selectedValue == 'ExamSiteChange' || selectedValue == 'Technical Glitch' || selectedValue == 'Pearson Merge Request'
           || selectedValue == 'Free Invoice' || selectedValue == 'Membership Switch'){
    ...
    OnclickMembershipSwitchNextButton: function (cmp, evt, helper) { 
        cmp.set("v.ErrorMessage",'');
        var oppInvoiceNum = cmp.get("v.oppInvoiceNumber");
        console.log('oppInvoiceNum ::>>   '+oppInvoiceNum);
        cmp.set("v.InvoiceNumber",oppInvoiceNum);
        if(oppInvoiceNum == null || oppInvoiceNum=='') { 
            cmp.set("v.ErrorMessage","Please Enter Invoice Number");  
        } else {
            helper.MembershipSwitchHelper(cmp, evt);
        }
    },
    
    OnclickOfNextButton: function (cmp, evt, helper) { 
        var invoiceNumber = cmp.find('oppInvoice').get('v.value');
        console.log('invoiceNumber::>>   '+invoiceNumber);
        if(invoiceNumber == null || invoiceNumber=='') { 
            cmp.set("v.ErrorMessage","Please Enter Invoice Number");
